...
/** ホーム上部の「最近の履歴」card を Compose 前に抑制するための runtime gate です。 */
public final class HomeRecentHistoryHooks {
    private HomeRecentHistoryHooks() {
    }

    /**
     * ホームの最近の履歴設定が ON の場合だけ「最近の履歴」card の renderer を抑制します。
    ...
    public static boolean shouldSuppress() {
        return HomeSuppressionGate.shouldSuppress(new HomeSuppressionGate.SuppressionState() {
            ...
            public boolean isSuppressionEnabled() {
                return LinimalConfig.get().isHomeRecentHistorySuppressionEnabled();
            }
        });
    }
}
